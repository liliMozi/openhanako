...
async function startServer() {
  ...
  if (existingInfo) {
    ...
    if (pidAlive) {
      ...
      try { process.kill(existingInfo.pid, "SIGKILL"); } catch {}
    }
...
function createTray() {
  ...
  icon.setTemplateImage(true); // macOS 自动适配深色/浅色模式
  tray = new Tray(icon);
...
app.on("window-all-closed", () => {
  // macOS：窗口关闭后 app 保持运行，点 dock 图标可以重新打开
...
app.on("before-quit", async (event) => {
  ...
  if (serverProcess && !serverProcess.killed) {
    ...
    // 复用路径：没有 serverProcess 引用，通过 PID 发 SIGTERM
    event.preventDefault();
    console.log("[desktop] 正在关闭复用的 Server...");
    try { process.kill(reusedServerPid, "SIGTERM"); } catch {}

    // 轮询等待进程退出（最多 5 秒）
    const deadline = Date.now() + 5000;
    while (Date.now() < deadline) {
      try { process.kill(reusedServerPid, 0); } catch { break; } // 进程已消失
      await new Promise(r => setTimeout(r, 200));
    }
    try { process.kill(reusedServerPid, "SIGKILL"); } catch {} // 超时则强制
