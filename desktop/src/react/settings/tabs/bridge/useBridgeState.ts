...
export function useBridgeState() {
  ...
  // 「正在输入」提示开关
  const [bridgeTypingNotification, setBridgeTypingNotification] = useState(true);

  // 读取「正在输入」提示开关
  useEffect(() => {
    hanaFetch('/api/preferences/bridge-typing')
      .then(r => r.json())
      .then(data => { if (typeof data.enabled === 'boolean') setBridgeTypingNotification(data.enabled); })
      .catch(() => {});
  }, []);

  const saveBridgeTypingNotification = async (enabled: boolean) => {
    try {
      await hanaFetch('/api/preferences/bridge-typing', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ enabled }),
      });
      setBridgeTypingNotification(enabled);
  ...
  return {
    ...
    bridgeTypingNotification, saveBridgeTypingNotification,
    tgToken, setTgToken,
