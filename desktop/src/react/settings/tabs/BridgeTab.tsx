...
export function BridgeTab() {
  ...
  return (
    <div className={`${styles['settings-tab-content']} ${styles['active']}`} data-tab="bridge">
      ...
      {/* 「正在输入」提示开关 */}
      <section className={styles['settings-section']}>
        <h2 className={styles['settings-section-title']}>{t('settings.agent.bridgeTypingNotification')}</h2>
        <div className="bridge-platform-header">
          <span className="bridge-readonly-desc">{t('settings.agent.bridgeTypingNotificationHint')}</span>
          <Toggle
            on={b.bridgeTypingNotification}
            onChange={(val) => b.saveBridgeTypingNotification(val)}
          />
        </div>
      </section>
