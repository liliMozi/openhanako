/**
 * full_agent — 调用完整能力的 Agent（替代阉割版 subagent）
 *
 * 异步 fire-and-forget + DeferredResultStore：和 subagent 完全一样的运作方式
 * 结果通过 deferred-result-ext 注入对话（steer 消息）
 * 进度通过 block_update 事件通知前端
 */
...
export async function execute(toolArgs, pluginCtx) {
  ...
  // fire-and-forget：hub 负责 deferred store + block_update
  bus.request("groupchat:execute-agent", {
    agentId,
    text: task,
    parentSessionPath,
    taskId,
  }, { timeout: TIMEOUT_MS }).catch(err => {
    console.error("[full_agent] request failed:", err.message);
  });

  // 模仿 subagent 的返回值格式——details 让前端渲染 block card
  return {
