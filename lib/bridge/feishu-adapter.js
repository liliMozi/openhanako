...
function extractFeishuResponseField(response, field) {
...
function requireFeishuResponseField(response, field, action) {
  const value = extractFeishuResponseField(response, field);
  if (!value) {
    throw new Error(`Feishu ${action} returned no ${field}`);
  }
  return value;
}
...
export function createFeishuAdapter({ appId, appSecret, agentId, onMessage, onStatus }) {
  ...
  const MESSAGE_DEDUPE_MAX = 1_000;
  const recentMessageIds = new Map();

  function hasRecentlySeenMessage(messageId) {
    if (!messageId) return false;

    const now = Date.now();
    for (const [id, seenAt] of recentMessageIds) {
      if (now - seenAt <= MESSAGE_DEDUPE_WINDOW_MS) break;
    ...
    recentMessageIds.set(messageId, now);
    while (recentMessageIds.size > MESSAGE_DEDUPE_MAX) {
      recentMessageIds.delete(recentMessageIds.keys().next().value);
    }
    return false;
  }
  ...
  const eventDispatcher = new lark.EventDispatcher({}).register({
    "im.message.receive_v1": async (data) => {
      ...
      if (hasRecentlySeenMessage(message.message_id)) {
        debugLog()?.log("bridge", `feishu duplicate message skipped: ${message.message_id}`);
        return;
      }
  ...
  return {
    ...
    async sendMediaBuffer(chatId, buffer, { mime, filename }) {
      if (mime.startsWith("image/")) {
        ...
        const imageKey = requireFeishuResponseField(res, "image_key", "image upload");
        await client.im.message.create({
          params: { receive_id_type: "chat_id" },
          data: {
        ...
        const fileKey = requireFeishuResponseField(res, "file_key", "file upload");
        await client.im.message.create({
          params: { receive_id_type: "chat_id" },
          data: {
