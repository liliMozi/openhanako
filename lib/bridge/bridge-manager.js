...
export class BridgeManager {
  ...
  async sendToSession(sessionKey, text) {
    ...
    // 查找对应的实例：优先用 _sessionInstanceMap 缓存，否则遍历所有匹配基础平台的实例
    let instanceId = this._sessionInstanceMap.get(sessionKey);
    if (!instanceId || !this._platforms.get(instanceId)?.adapter) {
      // 遍历所有匹配基础平台的已连接实例，取第一个
    ...
    const entry = this._platforms.get(instanceId);
    if (!entry?.adapter) {
      return { ok: false, error: `instance ${instanceId} not connected` };
    }

    // 使用缓存的真实 chatId（飞书等平台 chatId ≠ sessionKey 中的 ID）
    const chatId = this._chatIdCache.get(sessionKey) || fallbackChatId;

    debugLog()?.log("bridge", `← desktop takeover ${instanceId} chatId=${chatId} (${text.length} chars)`);

    // 1. 先把桌面用户的消息直接转发给外部平台用户（加 owner 前缀）
    try {
      await entry.adapter.sendReply(chatId, `owner: ${text}`);
      debugLog()?.log("bridge", `→ ${instanceId} forwarded owner message (${text.length} chars)`);
    } catch (err) {
      debugLog()?.error("bridge", `forward owner message failed (${instanceId}): ${err.message}`);
    }

    // 2. 广播 owner_echo（前端已本地渲染，用 owner_echo 方向标识避免重复）
    this._pushMessage({
      platform: basePlatform, instanceId, direction: "owner_echo", sessionKey,
      sender: "owner", text,
      isGroup: false, ts: Date.now(),
    });

    // 3. 异步发给 AI engine 处理，AI 回复通过 WebSocket 推送
    this._processDesktopTakeover(sessionKey, instanceId, chatId, text).catch(err => {
      debugLog()?.error("bridge", `desktop takeover async failed (${instanceId}): ${err.message}`);
    });
