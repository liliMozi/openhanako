...
import { extractTextContent } from "../../core/message-utils.js";
...
export class BridgeManager {
  ...
  async _handleMessage(platform, msg) {
    ...
    if (!msgAgentId) {
      console.warn(`[bridge] ${platform} 消息缺少 agentId，fallback 到焦点 agent "${agentId}"`);
    }
  ...
  /**
   * 桌面端在 owner bridge 会话中完成一轮对话后，将「桌面输入 + 助手回复」格式化后发到对应 IM。
   * 仅处理 SessionCoordinator 路径（微信进线走 executeExternalMessage，不会触发 chat turn_end，避免重复推送）。
   */
  async maybeSyncDesktopBridgeTurn(engine, sessionPath, { wasAborted, hadError } = {}) {
    if (!engine || !sessionPath || wasAborted || hadError) return;
    const norm = sessionPath.replace(/\\/g, "/");
    if (!norm.includes("/sessions/bridge/owner/")) return;

    const sessionKey = engine.findBridgeSessionKeyByPath?.(sessionPath);
    if (!sessionKey) return;

    const { platform, chatType, chatId, agentId: keyAgentId } = parseSessionKey(sessionKey);
    if (platform === "unknown" || !chatId) return;

    const sess = engine.getSessionByPath(sessionPath);
    const messages = sess?.messages;
    const { user, assistant } = this._lastUserAssistantFromMessages(messages);
    if (!user || !assistant) return;

    const cleanedAssistant = this._cleanReplyForPlatform(assistant);
    if (!cleanedAssistant) return;

    const body = this._formatDesktopBridgeTurnBody(user, cleanedAssistant);
    if (!body) return;

    const agentId = keyAgentId || engine.currentAgentId;
    const entry =
      this._platforms.get(this._getPlatformKey(platform, agentId)) ||
      this._findPlatformEntry(platform);
    if (!entry?.adapter?.sendReply || entry.status !== "connected") return;

    let targetChatId = chatId;
    if (platform === "qq" && entry.adapter.resolveOwnerChatId) {
      targetChatId = entry.adapter.resolveOwnerChatId(chatId) || chatId;
    }

    try {
      await entry.adapter.sendReply(targetChatId, body);
      debugLog()?.log("bridge", `→ ${platform} desktop turn sync (${body.length} chars)`);
      const agentObj = agentId && engine.getAgent?.(agentId);
      const sender = agentObj?.agentName || engine.agentName || "";
      this._pushMessage({
        platform,
        direction: "out",
        sessionKey,
      ...
      debugLog()?.warn("bridge", `desktop turn sync failed: ${msg}`);
      console.warn(`[bridge] desktop turn sync failed (${platform}): ${msg}`);
      const zh = getLocale().startsWith("zh");
      this._hub?.eventBus?.emit?.(
        {
          type: "notification",
          title: zh ? "桌面对话未能同步到社交平台" : "Desktop reply not synced to IM",
          body: msg,
        },
        null,
      );
  ...
  /** 取最近一轮 user → assistant 的可见文本（含用户侧图片提示） */
  _lastUserAssistantFromMessages(messages) {
    if (!Array.isArray(messages) || !messages.length) return { user: "", assistant: "" };
    let lastAsst = -1;
    ...
    if (lastAsst < 0) return { user: "", assistant: "" };
    let prevAsst = -1;
    ...
    if (lastUser < 0) return { user: "", assistant: "" };
    // 本轮没有新的 user（仅 steer 等延续上一条 assistant）则不同步，避免重复推送同一条「我：」
    if (prevAsst >= 0 && lastUser <= prevAsst) return { user: "", assistant: "" };
    const u = extractTextContent(messages[lastUser].content);
    const a = extractTextContent(messages[lastAsst].content, { stripThink: true });
    let userLine = (u.text || "").trim();
    if (u.images?.length) {
      const zh = getLocale().startsWith("zh");
      const tag = zh ? `[${u.images.length} 张图片]` : `[${u.images.length} image(s)]`;
      userLine = userLine ? `${userLine}\n${tag}` : tag;
    }
    return { user: userLine, assistant: (a.text || "").trim() };
  }

  _formatDesktopBridgeTurnBody(userText, assistantText) {
    const zh = getLocale().startsWith("zh");
    const pad = n => String(n).padStart(2, "0");
    const d = new Date();
    const time = `${pad(d.getHours())}:${pad(d.getMinutes())}`;
    if (zh) {
      return `[桌面 ${time}] 我：${userText}\n\n助手：${assistantText}`;
    }
    return `[Desktop ${time}] Me: ${userText}\n\nAssistant: ${assistantText}`;
  }
  ...
  _pushMessage(entry) {
    const sessionPath =
      entry.sessionPath ||
      (entry.sessionKey && this.engine.findBridgeSessionPathBySessionKey?.(entry.sessionKey)) ||
      undefined;
    const message = sessionPath ? { ...entry, sessionPath } : { ...entry };
    this._messageLog.push(message);
    if (this._messageLog.length > this._messageLogMax) {
      this._messageLog.shift();
    }
    this._hub.eventBus.emit(
      { type: "bridge_message", message },
      null,
    );
  }
