...
export class BridgeManager {
  ...
  async _handleMessage(platform, msg) {
    ...
    // 立刻发送"正在输入"，减少体感延迟（如果用户开启了这个开关）
    if (this.engine.preferences.getBridgeTypingNotification()) {
  ...
  async _flushGroupMessage(platform, chatId, sessionKey, line, meta, attachments, agentId) {
    ...
    // typing indicator（仅在用户开启时发送"正在输入"提示）
    if (this.engine.preferences.getBridgeTypingNotification()) {
  ...
  async _flushPending(sessionKey) {
    ...
    // typing indicator（Telegram 原生支持，其他平台忽略）（仅在用户开启时）
    if (this.engine.preferences.getBridgeTypingNotification()) {
