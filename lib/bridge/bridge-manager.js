...
const ADAPTER_REGISTRY = {
  ...
  qq: {
    create: (creds, onMessage, hooks) => createQQAdapter({
      appID: creds.appID, token: creds.token, onMessage,
      dmGuildMap: creds.dmGuildMap,
      onDmGuildDiscovered: hooks?.onQqDmGuild,
      onStatus: hooks?.onStatus,
    }),
    getCredentials: (cfg) => cfg?.enabled && cfg?.appID && cfg?.token
      ? { appID: cfg.appID, token: cfg.token, dmGuildMap: cfg.dmGuildMap }
      : null,
