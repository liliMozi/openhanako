 ...
 */
export function createCronScheduler({ cronStore, executeJob, onJobDone }) {
        ...
        try {
          {
            ...
            try {
              await Promise.race([
                executeJob(job),
                new Promise((_, reject) => { timer = setTimeout(() => reject(new Error("execution timeout (5min)")), EXEC_TIMEOUT); }),
              ]);
            } finally {
              clearTimeout(timer);
            }
