...
export function checkConfigYaml({ agentDir, hanakoHome }) {
  ...
  try {
    ...
    if (!content.includes(":")) throw new Error("不像 YAML 格式");
  } catch (err) {
    ...
    for (const tpl of templateCandidates) {
      try {
        if (fs.existsSync(tpl)) {
          ...
          return { fixed: true, message: `config.yaml 损坏 (${err.message})，已从模板重建` };
        }
      } catch {}
    }

    return { fixed: true, message: `config.yaml 损坏 (${err.message})，已备份为 ${path.basename(backupPath)}` };
  }
}
