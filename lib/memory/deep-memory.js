 ...
 * @param {string} configPath - config.yaml 路径（用于解析 API 凭证）
 * @param {string} utilityModel - utility 模型名
 * @returns {Promise<{ processed: number, factsAdded: number }>}
 */
export async function processDirtySessions(summaryManager, factStore, configPath, utilityModel) {
 ...
 */
async function extractFactsFromDiff(currentSummary, previousSnapshot, configPath, utilityModel) {
