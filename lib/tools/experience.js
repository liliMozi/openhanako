...
export function createExperienceTools(agentDir) {
  ...
  const recallTool = {
    ...
    label: "查看经验",
    description:
      "查看经验库。不传参数返回所有分类概览，传分类名返回该分类的具体经验。" +
      "当用户要求你完成一项具体工作（写代码、搜索资料、创建文档、分析问题等）时，" +
      "开始前应先调用此工具检查是否有相关经验可参考。" +
      "普通闲聊、问答、日常对话不需要调用。",
    parameters: Type.Object({
      category: Type.Optional(
        Type.String({ description: "分类名。不传则返回所有分类概览" }),
      ),
    }),
    execute: async (_toolCallId, params) => {
      ...
      if (!content.trim()) {
        return {
          content: [
            { type: "text", text: `没有找到分类「${category}」的经验。` },
          ],
          details: {},
        };
      }
  ...
  const recordTool = {
    ...
    parameters: Type.Object({
      category: Type.String({
        description:
          "经验所属分类，2-4 字短语，如「工具使用」「搜索技巧」「回答风格」",
      }),
      content: Type.String({
        description: "具体的经验内容，简洁直接，一句话",
      }),
    }),
    execute: async (_toolCallId, params) => {
      ...
      if (!category || !content) {
        return {
          content: [{ type: "text", text: "分类名和经验内容不能为空。" }],
          details: {},
        };
      }
      ...
      if (!result.added) {
        return {
          content: [{ type: "text", text: "这条经验已经记录过了。" }],
          details: {},
        };
      }
      ...
      return {
        content: [
          { type: "text", text: `已记录经验 [${category}]：${content}` },
        ],
